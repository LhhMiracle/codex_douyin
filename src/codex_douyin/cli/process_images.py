...
def _build_pipeline(args: argparse.Namespace) -> ImageProcessingPipeline:
...
def main() -> None:
    ...
    logger.info("Parsed final_url=%s", parsed_input.final_url or "<none>")
    logger.info("Parsed product_id=%s", parsed_input.product_id)

    if args.dry_run:
        logger.info("Dry run requested; skipping download and processing")
        return

    pipeline = _build_pipeline(args)
    fetch_target = parsed_input.final_url or parsed_input.product_id
